/* Verificação do PRÓXIMO NO PERCURSO (detail-next): abre o dossiê de um hero,
   carrega em next, e espera que o hero mude para o item seguinte do percurso.
...
const titleBefore = await previewTitle();
console.log('[hero] preview-title:', titleBefore);
if (!titleBefore) { console.error('FAIL: sem hero (preview-title vazio)'); process.exit(1); }
...
if (titleAfter === titleBefore) { console.error(`FAIL: título não mudou («${titleAfter}» = «${titleBefore}»)`); process.exit(1); }
